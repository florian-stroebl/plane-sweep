...
impl<'a, T: B> MyWidget<CodeViewState<'a, T>> for CodeView {
    ...
    fn ui(&mut self, ui: &mut eframe::egui::Ui, state: impl Into<CodeViewState<'a, T>>) {
        ...
        let text = "Initialize all data structures. Insert the first event into the beach line.";
        ...
        let text = "New Arc is left from the beach line. We insert the Site before the first leaf.";
        ...
        let text = "New Arc is right from the beach line. We insert the Site after the last leaf.";
        if matches!(s.typ, StepType::ReplaceLeft) {
            ui.label(RichText::new(text).heading().underline());
        } else {
            ui.label(RichText::new(text));
        ...
        let text = "Delete the leaf that disappears from the beach line.";
        if matches!(s.typ, StepType::ReplaceLeft) {
            ui.label(RichText::new(text).heading().underline());
        } else {
            ui.label(RichText::new(text));
        ...
        if let StepType::FullInfinite(hl, hr, l, r, v1, v2) = s.typ {
            ...
            ui.label(
                RichText::new(format!("Calculate the bisection between {l} and {r}."))
                    .heading()
                    .underline(),
            );
            ui.label(
                RichText::new(format!("Intersect the bisection with the boundary segments. Found new vertices Vertex {v1} and Vertex {v2}."))
                    .heading()
                    .underline(),
            );
            ui.label(RichText::new("Add Vertices to DCEL.").heading().underline());
        } else {
            ui.label("Calculate the bisection between left site and right site.");
            ui.label("Intersect the bisection with the boundary segments.");
        ...
        if let StepType::HalfInfinite(hl, hr, l, r, _, v1, v2) = s.typ {
            ...
            ui.label(
                RichText::new(format!("Determine the direction, to {l} and {r}. The direction is separated in UpLeft, DownLeft, DownRight and UpRight."))
                    .heading()
                    .underline(),
            );
            ui.label(
            ...
            ui.label(
                RichText::new("Intersect the bisection with the boundary segments that lie in the calculated direction.")
                    .heading()
                    .underline(),
            );
            ui.label(
