...
impl<T: B + From<f64>> Voronoi<T> {
            ...
            match event {
                event_queue::Event::Site(site) => {
                    ...
                    if above == leftest && site.x < self.beach[leftest].site().x {
                        ...
                        self.find_circle_event(middle, event, false);
                        ...
                        self.create_half_edges(middle, left, event);
                        self.find_circle_event(left, event, false);
                        self.find_circle_event(right, event, false);
                    }
                }
                event_queue::Event::Circle(key, Circle { x, y, r: _ }) => {
                    ...
                    let ml = self.dcel.get_twin(lm);
                    let rm = self.dcel.get_twin(mr);
                    ...
                    let (lr, rl) = self.create_half_edges(left, right, event);

                    // Set the center of the circle as a Vertex to the three sites.
                    self.dcel.set_origin(ml, vertex);
                    self.dcel.set_origin(rm, vertex);
                    self.dcel.set_origin(lr, vertex);

                    // (The next pointer are not used for the visiualization, therefore it is not that important if they are always correct)
                    // The face should point to the first vertex. the next pointer is the half-edge that stores the next vertex. The order should be clockwise. Using the twins of the edges and their next pointer it should be able to jump to the adjent site
                    self.dcel.set_next(lm, lr);
                    self.dcel.set_next(mr, ml);
                    self.dcel.set_next(rl, rm);

                    // 3.
                    self.find_circle_event(left, event, true);
                    self.find_circle_event(right, event, true);
                }
            }
        }
    }
    ...
    pub fn bound(&mut self, bound: BoundingBox<T>) {
        ...
        // We calculate the bisection

        // We intersect this bisection with all bounding sites, and take all unique points. the number of points must be two
        ...
        for [l, r] in twins {
            ...
            let line = Segment::<T>::new((s_a.x, s_a.y), (s_b.x, s_b.y)).bisection();
        ...
        for [l, r] in twins {
            ...
            // Construct bisection between site_a and site_b

            let s_a = self.sites[site_a];
            let s_b = self.sites[site_b];
            let s = Segment::<T>::new((s_a.x, s_a.y), (s_b.x, s_b.y));
            let line = s.bisection();
            let m = s.midpoint();
            let mdx = m.x - x;
            let mdy = m.y - y;
    ...
    pub fn find_circle_event(&mut self, key: SQKey, event_report: Event<T>, from_circle: bool) {
