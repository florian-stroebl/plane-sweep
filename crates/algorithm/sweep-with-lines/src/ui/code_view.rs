...
impl<'a, 'b, 'c, 'd, T: A> MyWidget<CodeViewState<'a, 'b, 'c, 'd, T>> for CodeView {
    ...
    fn ui(
        ...
        if let StepType::CalculateUpCpLp { up_cp_lp } = &s.typ {
            ...
            let text = RichText::new(format!(
                "  Calculate the combined set U(p) and C(p) and L(p): {seg}"
            ))
            .underline();
            ui.label(text);
        } else {
            ui.label("  Calculate the combined set U(p) and C(p) and L(p)");
        }
        ...
        if let StepType::InsertMergeQueue { idx } = &s.typ {
            ...
            let text = RichText::new(format!(
                "  Insert Intersection {intersection} into Merge Queue"
            ))
            .underline();
            ui.label(text);
        } else {
            ui.label("  Insert Intersection into Merge Queue");
        }
        if let StepType::Merge { seg, result, idx } = &s.typ {
            ...
            let text = RichText::new(format!("  Merging points: {result} at intersection between s{seg1} and {seg2}: result {res:?}")).underline();
            ui.label(text);
        } else {
            ui.label("  Merging points at intersection");
        }
    }
}
